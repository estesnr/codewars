...
function calculateYears(principal, interest, tax, desired) {
  ...
  let yrs = 0
  // compare the current principal to the desired value
  while(principal < desired) {
    // if it's less, add one to the year counter
    yrs++
    // calculate the accrued interest
    let accrued = principal * interest
    // add the accrued interest, minus the tax value to the principal
    principal += (accrued - (accrued * tax))
}
  // return the final counter once the loop is completed
  return yrs
  }
