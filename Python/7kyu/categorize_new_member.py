...
# Output
# Output will consist of a list of string values (in Haskell and C: Open or Senior) stating whether the respective member is to be placed in the senior or
# open category.
...
def open_or_senior(data):
  # create an empty list for the result
    bracket = []
    # begin looping through the input dictionary
    for x in data:
      # check for conditions to be put into the senior bracket
        if x[0] >= 55 and x[1] > 7:
          # if true, put entry "senior" into the result list
            bracket.append('Senior')
        else:
          # otherwise put "open" into the result list
            bracket.append('Open')
